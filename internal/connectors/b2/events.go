...
// EventSource is the poll-based source for a b2 connector. Spawns
// one poll loop per configured watcher; each loop has its own cadence
// and state-table scope key.
...
func (c *Connector) allWatchers() []connector.WatcherSpec {
	...
	if err != nil {
		slog.Warn("b2: watcher normalization failed; falling back to default-only",
